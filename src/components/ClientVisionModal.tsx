...
export function ClientVisionModal({ isOpen, onClose }: ClientVisionModalProps) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-xl p-4 md:p-8 flex items-center justify-center overflow-y-auto">
        <motion.div
        ...
        >
          ...
          {!isSubmitted ? (
            <div>
              {/* Header Badge */}
              <div className="flex items-center gap-2 text-xs font-mono tracking-widest text-zinc-400 uppercase mb-2">
              ...
              <p className="text-xs text-zinc-400 font-light mt-1 mb-6">
              ...
              <form onSubmit={handleSubmit}>
                ...
                {step === 1 && (
                  <motion.div
                    ...
                    className="space-y-4"
                  >
                    <label className="block text-xs uppercase font-mono tracking-wider text-zinc-400">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {availableServices.map((service) => {
                        ...
                        return (
                          ...
                          >
                            <div className={`p-2 rounded-xl border ${isSelected ? 'bg-white text-black border-white' : 'bg-zinc-900 text-zinc-400 border-zinc-800'}`}>
                              <Icon className="w-4 h-4" />
                            </div>
                            <div>
                              <h4 className="font-bold text-sm text-white uppercase">{service.label}</h4>
                              <p className="text-[11px] text-zinc-400 font-light mt-0.5">{service.desc}</p>
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <div className="pt-6 flex justify-end">
                      <button
                        ...
                        onClick={() => setStep(2)}
                        className="px-8 py-3.5 rounded-full bg-white text-black font-semibold text-xs uppercase tracking-wider flex items-center gap-2 hover:bg-zinc-200 transition-all disabled:opacity-40 shadow-lg active:scale-95"
                      >
                        <span>Next Step</span>
                  ...
                  >
                    ...
                    <div className="pt-6 flex items-center justify-between">
                      <button
                        type="button"
                        onClick={() => setStep(1)}
                        className="px-6 py-3 rounded-full text-xs font-semibold uppercase tracking-wider text-zinc-400 hover:text-white flex items-center gap-1.5"
                      >
            ...
            <div className="py-12 text-center space-y-6">
              <div className="w-16 h-16 rounded-full bg-white text-black flex items-center justify-center mx-auto shadow-2xl shadow-white/20">
              ...
              <p className="text-zinc-300 text-sm font-light max-w-md mx-auto leading-relaxed">
                Thank you, <span className="text-white font-bold">{clientName}</span>. At ODA, we take time to understand before we create. Our team is now reviewing your responses and will reach out with a refined creative proposal.
              </p>

              <button
                onClick={resetAndClose}
                className="px-8 py-3.5 rounded-full bg-zinc-900 border border-zinc-700 text-white font-semibold text-xs uppercase tracking-wider hover:bg-zinc-800 transition-all"
              >
                Close Window
              </button>
            </div>
